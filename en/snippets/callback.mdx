Notifications are sent using `POST Content-Type:"application/json"` method.

<Warning>
  For your project to receive callbacks, ensure your project accepts free (unfiltered) HTTP POST requests from the pool of IP addresses:
...
<Expandable title="RSA-SHA256" defaultOpen="true">
  ...
  - `X-RSA-KEY-ID` - identifier of the used `RSA` key
  - `X-RSA-NONCE` - random `nonce` string, unique for each request
  - `X-RSA-SHA256-SIGN` - message signature in `base64` encoding

  #### **Signature Calculation Algorithm:**

  1. The used `RSA` key is selected and written to the `X-RSA-KEY-ID` header
  2. A random string is generated and written to the `X-RSA-NONCE` header
  3. The `body` is formed - binary representation of the **serialized body** of the HTTP message in `UTF-8` encoding - exactly what will be sent over the network, after serialization
  ...
  5. The obtained signature `sign` is encoded in `base64` and written to the `X-RSA-SHA256-SIGN` header

  #### **Signature Verification Algorithm:**

  1. Select the public `RSA` key by the `X-RSA-KEY-ID` header value (can be automatically retrieved by calling CardRouting API, keys should be cached)
  2. Take the `nonce` value from the `X-RSA-NONCE` header
  3. Take the `body` - binary representation of the **serialized body** of the HTTP message in `UTF-8` encoding - exactly what was received over the network, before deserialization
  4. Convert the `X-RSA-SHA256-SIGN` value from `base64` to binary representation `sign`
  5. Verify the signature `sign` for the message `nonce`\+`body` using `RSA` with `SHA256` hashing algorithm and signature format `PKCS#1 v1.5`
  6. The message is considered authenticated if the signature verification is successful
</Expandable>

<Expandable title="ECDSA-SHA256">
  For ECDSA, the curve `prime256v1` (also known as `secp256r1` and `NIST P256`) is used.

  ECDSA signature information is located in the HTTP message headers:

  - `X-ECDSA-KEY-ID` - identifier of the used `ECDSA` key
  - `X-ECDSA-SHA256-SIGN` - message signature in `base64` encoding

  #### **Signature Calculation Algorithm:**

  1. The used `ECDSA` key is selected and written to the `X-ECDSA-KEY-ID` header
  2. The `body` is formed - binary representation of the **serialized body** of the HTTP message in `UTF-8` encoding - exactly what will be sent over the network, after serialization
  ...
  4. The obtained signature `sign` is encoded in `base64` and written to the `X-ECDSA-SHA256-SIGN` header

  #### **Signature Verification Algorithm:**

  1. Select the public `ECDSA` key by the `X-ECDSA-KEY-ID` header value (can be automatically retrieved by calling CardRouting API, keys should be cached)
  2. Take the `body` - binary representation of the **serialized body** of the HTTP message in `UTF-8` encoding - exactly what was received over the network, before deserialization
  3. Convert the `X-ECDSA-SHA256-SIGN` value from `base64` to binary representation `sign`
  4. Verify the signature `sign` for the message `body` using `ECDSA` with `SHA256` hashing algorithm and signature format `IETF RFC 3279`
  5. The message is considered authenticated if the signature verification is successful
</Expandable>

#### Examples of Callback Handlers with Signature Verification
...
$finmar_login = 'YOUR_LOGIN';
$finmar_password = 'YOUR_PASSWORD';
...
const config = {
    ...
    finmarAuth: Buffer.from('YOUR_LOGIN:YOUR_PASSWORD').toString('base64'),
