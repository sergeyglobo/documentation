...
description: " "
sidebarTitle: "Подтверждение обработки операции"
---
...
```mermaid
sequenceDiagram
    participant API as Finmart API
    participant Server as Ваш сервер
    participant DB as База данных

    API->>Server: Webhook уведомление
    Server->>DB: Проверка дубликата
    alt Операция не обработана
        Server->>DB: Зачисление средств
        Server->>DB: Сохранение операции
        Server->>API: Подтверждение операции
    end
    
    Note over Server: Периодический опрос (каждые 5 мин)
    Server->>API: GET /client_unconfirmed_success_payments
    API-->>Server: Массив операций
    loop Для каждой операции
        Server->>DB: Обработка операции
        Server->>API: Подтверждение операции
    end
```
