...
description: " "
sidebarTitle: "Перенаправление пользователя на платёжную страницу"
---
...
При `result.launch_type=redirect`, рекомендуемый способ показа страницы - перенаправление пользователя в текущей вкладке браузера:
...
**Важно:** Некоторые платёжные провайдеры могут некорректно работать в iframe из-за политики безопасности. 
В этом случае используйте прямое перенаправление или открытие в новой вкладке/окне.

### Открытие в новой вкладке/окне
При `result.launch_type=new_window`, следует **обязательно** открывать платёжную страницу в новой вкладке:
