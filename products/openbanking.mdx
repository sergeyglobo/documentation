...
<Steps>
  <Step title="Выбор метода Open Banking">
    Клиент выбирает иконку **Open Banking** в интерфейсе сайта проекта.
  </Step>
...
    **Важно:** используйте `"method": "openbanking"`.

    <Accordion title="Пример запроса создания" icon="terminal">
      ```bash Запрос создания операции
      curl --location '[https://sandbox.finmar.tech/prepare2/api/v3/checkout](https://sandbox.finmar.tech/prepare2/api/v3/checkout)' \
...
          }
      },
      "url_callback": "[https://finmar.requestcatcher.com/?callback](https://finmar.requestcatcher.com/?callback)",
      "url_redirect_success": "[https://finmar.requestcatcher.com](https://finmar.requestcatcher.com)?success",
      "url_redirect_fail": "[https://finmar.requestcatcher.com](https://finmar.requestcatcher.com)?fail"
      }'
      ```
    </Accordion>
  </Step>
  <Step title="Получение URL перенаправления">
    Finmar API создает уникальный URL для перенаправления (на виджет или напрямую на банк) и возвращает его сайту проекта.
...
      "success": true,
      "result": {
          "url": "[https://card.cheipho.com/transaction/a32f658c-df98-4fc2-8bad-cbcf49aa8ee3](https://card.cheipho.com/transaction/a32f658c-df98-4fc2-8bad-cbcf49aa8ee3)",
...
  </Step>
  <Step title="Проведение операции Open Banking">
    Клиент проходит процедуру оплаты/перевода через свой банк (или виджет) и подтверждает условия оферты.
  </Step>
  <Step title="Уведомление о завершении операции">
    Finmar API отправляет сайту проекта уведомление о завершении транзакции. **payment_product** будет равен **openbanking**.
...
            "url_callback": "[https://api.fake-pay.services/pg/callback/a1b2c3d4-e5f6-7890-1234-567890abcdef/deposit](https://api.fake-pay.services/pg/callback/a1b2c3d4-e5f6-7890-1234-567890abcdef/deposit)",
            "payment_profile": "test_integration_fake",
            "url_redirect_fail": "[https://api.fake-pay.services/redirect/result/a1b2c3d4-e5f6-7890-1234-567890abcdef/deposit/5678901/failure](https://api.fake-pay.services/redirect/result/a1b2c3d4-e5f6-7890-1234-567890abcdef/deposit/5678901/failure)",
            "client_operation_id": "5678901",
            "operation_display_id": "5678901",
            "url_redirect_success": "[https://api.fake-pay.services/redirect/result/a1b2c3d4-e5f6-7890-1234-567890abcdef/deposit/5678901/success](https://api.fake-pay.services/redirect/result/a1b2c3d4-e5f6-7890-1234-567890abcdef/deposit/5678901/success)"
          },
          "rates": {
